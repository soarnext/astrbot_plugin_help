# astrbot_plugin_xirohelp

<div align="center">

![:astrbot_plugin_xirohelp](https://count.getloli.com/@:astrbot_plugin_xirohelp?theme=minecraft)
...
`astrbot_plugin_xirohelp` 是一款为 AstrBot 打造的帮助菜单插件。它能够自动扫描所有已激活插件的指令，并生成一张精美的帮助图片。支持权限分离、自定义头像、名称映射等丰富功能。
...
| `/adminhelps` | 管理帮助、管理菜单、后台菜单 | 管理员 | 生成管理员帮助图（仅含管理员指令） |

## ⚙️ 配置项介绍

| 配置项 | 说明 | 默认值 |
| :--- | :--- | :--- |
| `bot_name` | 机器人名称 (自动生成标题) | `AstrBot` |
| `help_subtitle` | 帮助菜单副标题 | `(空)` |
| `admin_help_subtitle` | 管理员帮助副标题 | `(空)` |
| `help_avatar` | 菜单左上角头像 | `(默认 Logo)` |
| `plugin_blacklist` | 插件黑名单 | `["builtin_commands"]` |
| `plugin_name_map` | 插件显示名称映射 | `[]` |
| `show_builtin_cmds` | 是否显示内部指令 | `true` |
| `admin_commands` | 管理员指令强制列表 | `[]` (强制设为管理员指令) |
| `user_commands` | 普通指令强制列表 | `[]` (强制设为普通指令) |
| `custom_cmds` | 自定义额外命令 | `[]` |
